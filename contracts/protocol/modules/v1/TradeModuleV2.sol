...
contract TradeModuleV2 is ModuleBase, ReentrancyGuard {
    ...
    modifier onlyManagerWhitelistedAndValidSet(ISetToken _setToken) {
        _customValidate(_setToken);
        _;
    }
    ...
    function trade(
        ...
        external
        nonReentrant
        onlyManagerWhitelistedAndValidSet(_setToken)
    {
    ...
    // modification of ModuleBase._validateOnlyManagerAndValidSet
    function _customValidate(ISetToken _setToken) internal view {
        bool valid = isSetManager(_setToken, msg.sender) || isWhitelisted(_setToken,msg.sender);
        require(valid, "Must be the SetToken manager or whitelisted");
